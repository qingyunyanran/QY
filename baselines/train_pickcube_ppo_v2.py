"""
PickCube-v1 PPO 训练 v2 - state_dict + flatten wrapper 匹配 CL 实验环境
"""
import os
import sys
...
SAVE_DIR = "E:\\munich\\rl_baselines\\pick_cube_v2"
...
final_model_path = os.path.join(SAVE_DIR, 'pick_cube_final.zip')
...
for ep in range(n_episodes):
    ...
    while not done:
        flat = obs.copy()
        norm_obs = np.clip(
            (flat - obs_rms.mean) / np.sqrt(obs_rms.var + eps),
            -clip_obs, clip_obs
        ).astype(np.float32)
