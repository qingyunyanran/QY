...
ManiSkill3 CL - Route A v9
  ...
  Why v9:
    ...
    - LoRA rank=8 on each policy_net layer (~12K extra params)
...
# v9 hyperparams
LR_START = 3e-4          # moderate: training LoRA + heads + value_net
LR_END = 3e-5
ENT_COEF = 0.01
N_STEPS = 2048
BATCH_SIZE = 128
...
if __name__ == "__main__":
    print("=" * 80)
    print("  ManiSkill3 CL - Route A v9 (Frozen Backbone + LoRA + Task Heads)")
    print("=" * 80)
    ...
    ckpt0_path = os.path.join(CKPT_SAVE_DIR, "route_a_v9_phase0_pokecube.pt")
    ...
    while total_steps_done < TOTAL_TIMESTEPS:
        ...
        if avg_sr > best_avg_sr:
            best_avg_sr = avg_sr
            ckpt_best = os.path.join(CKPT_SAVE_DIR, "route_a_v9_best_avg.pt")
            torch.save({
        ...
        if sr_t1 > best_pickcube_sr:
            ...
            ckpt_best_pc = os.path.join(CKPT_SAVE_DIR, "route_a_v9_best_pickcube.pt")
            torch.save({
                "state_dict": model.policy.state_dict(),
                "pokecube_head": pokecube_head,
                "lora_rank": LORA_RANK,
        ...
        if total_steps_done >= int(TOTAL_TIMESTEPS * 0.3) and best_pickcube_sr == 0.0:
            if not no_sr_warning_fired:
                ...
                ckpt_diag = os.path.join(CKPT_SAVE_DIR, "route_a_v9_30pct_diag.pt")
                torch.save({
                    "state_dict": model.policy.state_dict(),
                    "pokecube_head": pokecube_head,
                }, ckpt_diag)
        ...
        result_file = os.path.join(RESULT_DIR, "cl_results_route_a_v9_partial.json")
    ...
    # Save final
    ckpt1_path = os.path.join(CKPT_SAVE_DIR, "route_a_v9_final.pt")
    torch.save({
        "state_dict": model.policy.state_dict(),
        "pokecube_head": pokecube_head,
        "lora_rank": LORA_RANK,
    ...
    result_file = os.path.join(RESULT_DIR, "cl_results_route_a_v9.json")
    with open(result_file, "w") as f:
        json.dump(all_results, f, indent=2, default=float)

    print(f"\n{'='*80}")
    print(f"  Route A v9 Results: {task0} -> {task1}")
    print(f"{'='*80}")
